...
charts.response(charts.ofat(df, "ctx_noise"), "ctx_noise", "g_1shot")  # a knob sweep (mean ± SEM)
charts.scatter(df, "edge_val_std", "g_1shot", color="ctx_noise")       # a metric vs the outcome
...
```python
charts.response(charts.ofat(df, "sim_horizon"), "sim_horizon", "g_1shot_fair", save="my.png")
```
...
for ax, k in zip(axes.ravel(), knobs):
    charts.response(charts.ofat(df, k), k, "g_1shot", ax=ax)
fig.tight_layout()
```
...
| `correlate --out-dir` | `metric_outcome.csv`, `factor_outcome.csv`, `importance_g_1shot.csv`, `importance_g_shootN.csv` |
